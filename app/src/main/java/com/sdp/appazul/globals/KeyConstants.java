package com.sdp.appazul.globals;

public class KeyConstants {
    private KeyConstants() {
    }

    public static final String EXCEPTION_LABEL = "Exception";
    public static final String STATUS_KEY = "status";
    public static final String FAIL_KEY = "fail";
    public static final String SUCCESS_KEY = "ok";
    public static final String TRANSFORMATION = "RSA/ECB/PKCS1Padding";
    public static final String ACCESS_TOKEN_LABEL = "access_token";
    public static final String CONTENT_TYPE = "Content-Type";
    public static final String TEXT_JSON = "text/json";
    public static final String SSC_KEY = "x-ssc-key";
    public static final String AUTH_TOKEN = "X-AUTH-TOKEN";
    public static final String SHA_256 = "RSA/None/OAEPWithSHA-256AndMGF1Padding";


    public static final String DD_MM_YYYY_KEY = "dd/MM/yyyy";
    public static final String YYYY_MM_DD_KEY = "yyyy-MM-dd";
}
