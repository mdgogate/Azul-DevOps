...
import com.sdp.appazul.adapters.GridViewAdpater;
...
public class DashBoardActivity extends BaseLoggedInActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.getStringExtra("GENERATE_LINK") != null) {
            link = intent.getStringExtra("GENERATE_LINK");
            if (link.equalsIgnoreCase("LINK")) {
                PaymentLinkBottomSheet sheet = new PaymentLinkBottomSheet(locationJson);
                sheet.show(getSupportFragmentManager(), Constants.BOTTOM_SHEET);
            }
        }
    ...
    private void getWidgetDataFromAPi(int locationType, String lastLocationMid) {
        ...
        try {
            ...
            json.put("tcp", RSAHelper.ecryptRSA(DashBoardActivity.this, tcpKey));
            JSONObject payload = new JSONObject();
            payload.put("device", DeviceUtils.getDeviceId(this));
            JSONObject pagosrequest = new JSONObject();
            pagosrequest.put("merchantIdList", lastLocationMid);
        ...
        if (locationType == 0) {
            apiManager.callAPI(ServiceUrls.TRANSACTION_WIDGET, json);
        } else {
            apiManager.callAPI(ServiceUrls.TRANSACTION_WIDGET_QR, json);
        }
...
//        super.onBackPressed();
    ...
    private void initControls() {
        ...
        dashBoardBurgerMenu.setOnClickListener(dashBoardBurgerMenuView -> {
            DashboardMenuBottomsheet bottomsheet = new DashboardMenuBottomsheet(locationJson);
        ...
        if (!TextUtils.isEmpty(lastLocationNamee)) {
            ...
            int locaType;
            if (lastLocationType.equalsIgnoreCase("AssignedLocations")) {
                locaType = 0;
            } else {
                locaType = 1;
            }
            getWidgetDataFromAPi(locaType, lastMid);
        } else {
            locationTxt.setText("Sucursal 6 Noviembre");
            getWidgetDataFromAPi(0, "99999995");
        }
    ...
    public void setContent(String locationType, String midSelected, String content, int i) {
        locationTxt.setText(content);

        int locaType;
        if (locationType.equalsIgnoreCase("AssignedLocations")) {
            locaType = 0;
        } else {
            locaType = 1;
        }
        ...
        if (i == 1) {
            getWidgetDataFromAPi(locaType, midSelected);
            bottomsheet.dismiss();
        }
    }
    ...
    private void parseJsonData(String responseString) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                models.add(new Model(period, transactionSum, transactionCount, amountToReceive, discount, dateFrom, dateTo));
