...
public class DialogCalendarAdapter extends BaseAdapter {
    ...
    public DialogCalendarAdapter(Context c, GregorianCalendar monthCalendar, boolean isIntialDate1,
                                 String selectedInitialDateString, String selectedFinalDateString) {
        Locale.setDefault(Locale.US);
        DialogCalendarAdapter.dayString = new ArrayList<>();
        month = monthCalendar;
    ...
    public void modifyCalLayout(String gridvalue, int position) {
        if (!initString.equalsIgnoreCase(finalString)) {
            if (utils.isBetweenDates(initString + milisec, finalString + milisec,
                    date + milisec)) {
